...
- `transcript_diarized_json`: transcripción JSON con etiquetas de hablante (requiere `WHISPER_ASR_URL` apuntando a whisper-asr).
...
- `transcript_json`, `transcript_text`, `transcript_srt`, `transcript_diarized_json`: salidas de transcripción.
...
- `multipart/form-data` con campos `file` y `media_format` (`transcript_json`, `transcript_text`, `transcript_srt` o `transcript_diarized_json`).
- Usa `transcript_diarized_json` como media_format para obtener etiquetas de hablante si whisper-asr está configurado.
